...
export const signupUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(201).json({ success: true, message: 'Signup successful. Please verify your email.', ...result });
});
...
export const forgotPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(200).json({ success: true, message: 'If that email exists, a password reset link has been sent.', ...(token ? { resetToken: token } : {}) });
});
